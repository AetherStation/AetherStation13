...
/obj/item/bodypart/proc/dismember(dam_type = BRUTE, silent=TRUE)
	...
	affecting.receive_damage(clamp(brute_dam/2 * affecting.body_damage_coeff, 15, 50), clamp(burn_dam/2 * affecting.body_damage_coeff, 0, 50), should_wound = FALSE) //Damage the chest based on limb's existing damage
	if(!silent)
 ...
 */
/obj/item/bodypart/proc/try_dismember(wounding_type, wounding_dmg)
