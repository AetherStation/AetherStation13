...
/datum/reagent/medicine/leporazine/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	if(M.bodytemperature > target_temp)
		M.adjust_bodytemperature(-40 * TEMPERATURE_DAMAGE_COEFFICIENT * REM, target_temp)
	else if(M.bodytemperature < (target_temp + 1))
		M.adjust_bodytemperature(40 * TEMPERATURE_DAMAGE_COEFFICIENT * REM, 0, target_temp)
	if(ishuman(M))
		var/mob/living/carbon/human/humi = M
		if(humi.coretemperature > target_temp)
			humi.adjust_coretemperature(-40 * TEMPERATURE_DAMAGE_COEFFICIENT * REM, target_temp)
		else if(humi.coretemperature < (target_temp + 1))
			humi.adjust_coretemperature(40 * TEMPERATURE_DAMAGE_COEFFICIENT * REM, 0, target_temp)
	..()
...
/datum/reagent/medicine/adminordrazine/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	M.adjustToxLoss(-5 * REM, FALSE, TRUE)
	M.setOxyLoss(0, 0)
	M.setCloneLoss(0, 0)
	...
	M.reagents.remove_all_type(/datum/reagent/toxin, 5 * REM, FALSE, TRUE)
...
/datum/reagent/medicine/synaptizine/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	M.AdjustStun(-20 * REM )
	M.AdjustKnockdown(-20 * REM )
	M.AdjustUnconscious(-20 * REM )
	M.AdjustImmobilized(-20 * REM )
	M.AdjustParalyzed(-20 * REM )
	if(holder.has_reagent(/datum/reagent/toxin/mindbreaker))
		holder.remove_reagent(/datum/reagent/toxin/mindbreaker, 5 * REM)
	M.hallucination = max(M.hallucination - (10 * REM), 0)
	if(prob(30))
		M.adjustToxLoss(1, 0)
		. = TRUE
	..()
...
/datum/reagent/medicine/synaphydramine/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	if(holder.has_reagent(/datum/reagent/toxin/histamine))
		holder.remove_reagent(/datum/reagent/toxin/histamine, 5 * REM)
	M.hallucination = max(M.hallucination - (10 * REM), 0)
	if(prob(30))
		M.adjustToxLoss(1, 0)
		. = TRUE
...
/datum/reagent/medicine/cryoxadone/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	if(M.bodytemperature < T0C)
		M.adjustOxyLoss(-3 * power * REM, 0)
		M.adjustBruteLoss(-power * REM, 0)
		M.adjustFireLoss(-power * REM, 0)
		M.adjustToxLoss(-power * REM, 0, TRUE) //heals TOXINLOVERs
		M.adjustCloneLoss(-power * REM, 0)
		for(var/i in M.all_wounds)
			var/datum/wound/iter_wound = i
			iter_wound.on_xadone(power * REAGENTS_EFFECT_MULTIPLIER)
...
/datum/reagent/medicine/clonexadone/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	if(M.bodytemperature < T0C)
		M.adjustCloneLoss((0.00006 * (M.bodytemperature ** 2) - 6) * REM, 0)
...
/datum/reagent/medicine/pyroxadone/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	if(M.bodytemperature > BODYTEMP_HEAT_DAMAGE_LIMIT)
		...
		M.adjustOxyLoss(-2 * power * REM, FALSE)
		M.adjustBruteLoss(-power * REM, FALSE)
		M.adjustFireLoss(-1.5 * power * REM, FALSE)
		M.adjustToxLoss(-power * REM, FALSE, TRUE)
		M.adjustCloneLoss(-power * REM, FALSE)
		for(var/i in M.all_wounds)
			var/datum/wound/iter_wound = i
			iter_wound.on_xadone(power * REAGENTS_EFFECT_MULTIPLIER )
		REMOVE_TRAIT(M, TRAIT_DISFIGURED, TRAIT_GENERIC)
...
/datum/reagent/medicine/salglu_solution/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	if(M.blood_volume < maximum_reachable) //Can only up to double your effective blood level.
		...
		M.blood_volume = new_blood_level + (extra_regen * REM)
	if(prob(33))
...
/datum/reagent/medicine/calomel/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	for(var/datum/reagent/toxin/R in M.reagents.reagent_list)
		M.reagents.remove_reagent(R.type, 3 * REM )
	if(M.health > 20)
		M.adjustToxLoss(1 * REM , 0)
		. = TRUE
	..()
...
/datum/reagent/medicine/potass_iodide/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	if(M.radiation > 0)
		M.radiation -= min(8 * REM , M.radiation)
	..()
...
/datum/reagent/medicine/pen_acid/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	M.radiation -= (max(M.radiation - RAD_MOB_SAFE, 0) / 50) * REM
	M.adjustToxLoss(-2 * REM , 0)
	for(var/datum/reagent/R in M.reagents.reagent_list)
		if(R != src)
			M.reagents.remove_reagent(R.type, 2 * REM )
	..()
	. = TRUE
...
/datum/reagent/medicine/diphenhydramine/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	if(DT_PROB(5, delta_time))
		M.drowsyness++
	M.jitteriness -= 1 * REM
	holder.remove_reagent(/datum/reagent/toxin/histamine, 3 * REM )
	..()
...
/datum/reagent/medicine/epinephrine/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	if(holder.has_reagent(/datum/reagent/toxin/lexorin))
		...
		holder.remove_reagent(/datum/reagent/medicine/epinephrine, 1 * REM )
		if(prob(20))
...
/datum/reagent/medicine/insulin/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	if(M.AdjustSleeping(-20 * REM ))
		. = TRUE
	holder.remove_reagent(/datum/reagent/consumable/sugar, 3 * REM )
	..()
...
/datum/reagent/medicine/earthsblood/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	if(current_cycle <= 25) //10u has to be processed before u get into THE FUN ZONE
		...
		M.jitteriness = clamp(M.jitteriness + (3 * REM ), 0, 30)
	...
	M.druggy = clamp(M.druggy + (10 * REM ), 0, 15 * REM ) //See above
	..()
	. = TRUE
...
/datum/reagent/medicine/changelingadrenaline/on_mob_life(mob/living/carbon/metabolizer, delta_time, times_fired)
	..()
	metabolizer.AdjustAllImmobility(-20 * REM )
	metabolizer.adjustStaminaLoss(-10 * REM , 0)
	metabolizer.Jitter(10 * REM )
	metabolizer.Dizzy(10 * REM )
	return TRUE
...
/datum/reagent/medicine/changelinghaste/on_mob_life(mob/living/carbon/metabolizer, delta_time, times_fired)
	metabolizer.adjustToxLoss(2 * REM , 0)
	..()
	return TRUE
...
/datum/reagent/medicine/modafinil/on_mob_life(mob/living/carbon/metabolizer, delta_time, times_fired)
	if(!overdosed) // We do not want any effects on OD
		overdose_threshold = overdose_threshold + ((rand(-10, 10) / 10) * REM ) // for extra fun
		metabolizer.AdjustAllImmobility(-5 * REM )
		metabolizer.adjustStaminaLoss(-0.5 * REM , 0)
...
/datum/reagent/medicine/modafinil/overdose_process(mob/living/M, delta_time, times_fired)
	...
	switch(overdose_progress)
		if(1 to 40)
			M.jitteriness = min(M.jitteriness + (1 * REM ), 10)
			M.stuttering = min(M.stuttering + (1 * REM ), 10)
			M.Dizzy(5 * REM )
			if(prob(50))
				M.losebreath++
		if(41 to 80)
			...
			M.jitteriness = min(M.jitteriness + (1 * REM ), 20)
			M.stuttering = min(M.stuttering + (1 * REM ), 20)
			M.Dizzy(10 * REM )
			if(prob(50))
				M.losebreath++
			if(prob(20))
...
/datum/reagent/medicine/psicodine/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	M.jitteriness = max(M.jitteriness - (6 * REM ), 0)
	M.dizziness = max(M.dizziness - (6 * REM ), 0)
	M.set_confusion(max(M.get_confusion() - (6 * REM ), 0))
	M.disgust = max(M.disgust - (6 * REM ), 0)
...
/datum/reagent/medicine/granibitaluri/overdose_process(mob/living/M, delta_time, times_fired)
	...
	M.adjustToxLoss(0.2 * REM , FALSE) //Only really deadly if you eat over 100u
	..()
...
/datum/reagent/medicine/coagulant/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	if(bloodiest_wound)
		if(!was_working)
			...
			was_working = TRUE
		bloodiest_wound.blood_flow = max(0, bloodiest_wound.blood_flow - (clot_rate * REM ))
	else if(was_working)
		was_working = FALSE
